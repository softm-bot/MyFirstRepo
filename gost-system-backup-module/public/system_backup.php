...
if (isset($_GET['view'])) {
    // opened via index router — relative links to index
    $homeHref = './';
...
<section class="panel">
  <h2>1. Пароль архива</h2>
  <p class="hint">ZIP с бэкапом всегда запаролен. Пароль задаёт только администратор. Хранится в БД в зашифрованном виде.</p>
  <p>Статус: <?php if ($hasPass): ?><span class="badge">пароль задан</span><?php else: ?><span class="badge" style="background:#fbeae9;color:#b3261e">пароль не задан</span><?php endif; ?></p>
...
<section class="panel">
  <h2>2. Создать полный бэкап сейчас</h2>
  <p class="hint">В архив входит: исходный код, storage с файлами документов, полный SQL всех таблиц, инструкция RESTORE.txt.</p>
  <form method="post">
    <input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>">
...
<section class="panel">
  <h2>3. Готовые архивы</h2>
  <?php if (!$list): ?>
    <p class="hint">Пока нет созданных бэкапов.</p>
  <?php else: ?>
  <table>
    <thead><tr><th>Файл</th><th>Размер</th><th>Создан (UTC)</th><th></th></tr></thead>
    <tbody>
    <?php foreach ($list as $item): ?>
      <tr>
        <td><code><?= htmlspecialchars($item['file']) ?></code></td>
        <td><?= htmlspecialchars(number_format($item['size'] / 1048576, 2, '.', ' ')) ?> МБ</td>
        <td><?= htmlspecialchars(gmdate('Y-m-d H:i', $item['mtime'])) ?></td>
        <td class="row">
...
<section class="panel">
  <h2>4. Еженедельный cron</h2>
  <p class="hint">Добавьте задание в панели хостинга (Cron). Раз в неделю в 03:00:</p>
  <pre>0 3 * * 0 /usr/bin/php <?= htmlspecialchars($cronPath) ?></pre>
  <p class="hint">Скрипт создаст запароленный архив в <code>storage/backups/</code> и оставит последние 8 копий. Пароль берётся из настроек администратора.</p>
</section>

<section class="panel">
  <h2>5. Развёртывание на новом хостинге</h2>
  <ol class="hint">
    <li>Скачайте ZIP и распакуйте с паролем.</li>
    ...
    <li>Выдайте права на запись в <code>storage/</code>.</li>
